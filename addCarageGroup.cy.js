...
it('addCarageGroup', function () {
    ...
    cy.get('[data-testid="name"]').type(this.carageGroup.addGroup.nameGroup);
    cy.get('[data-testid="autocomplete-railway-administration"]').click();
    cy.contains(this.carageGroup.addGroup.nameRoelway).should('be.visible').click();
    cy.get('[data-testid="autocomplete-passenger-carriers"]').click();
    cy.contains(this.carageGroup.addGroup.nameCarier).should('be.visible').click();
    cy.get('[data-testid="autocomplete-branch"]').click();
    cy.contains(this.carageGroup.addGroup.nameBranch).should('be.visible').click();
    cy.get('[data-testid="autocomplete-structural-subdivision"]').click();
    cy.contains(this.carageGroup.addGroup.nameRegistered).should('be.visible').click();
